 ...
 * The CONSUMER sends `(did, serviceAgreementId, serviceDefinitionId, signature, consumerAddress`) to the service endpoint (BRIZO). 
 The DID tells the PUBLISHER which asset to serve under these terms. `serviceDefinitionId` tells the PUBLISHER where to find the 
 preimage to verify the signature. 
    ```
 ...
 * The CONSUMER initialize the agreement directly via `ocean.agreements.create` from the Squid library. 
